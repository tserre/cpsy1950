...
- ✔ **+ 2 pts** — Describes encoding models as predicting brain responses from model activations. Full credit for any reasonable description that captures the regression/prediction idea, even without technical detail.
- **+ 1.5 pts** — Mentions prediction but vague (e.g., "uses the model to predict brain activity" without specifying regression, mapping, or what's being predicted).
- **+ 0.75 pts** — Partial or confused (e.g., conflates encoding with decoding, or gives only a label with no description).
- **+ 0 pts** — Missing or incorrect.

**1b. Description of RSA (2 pts)**

- ✔ **+ 2 pts** — Describes RSA as comparing similarity or distance structures between model and brain. Full credit for any description that captures the pairwise comparison idea, even without saying "geometry."
- **+ 1.5 pts** — Mentions comparison but vague (e.g., "sees if the model and brain represent things similarly" without specifying pairwise structure or similarity matrices).
- **+ 0.75 pts** — Partial or confused (e.g., describes RSA as a type of encoding model, or gives only a label).
- **+ 0 pts** — Missing or incorrect.
...
- ✔ **+ 0.75 pts** — Identifies a real difference but explains it loosely or incompletely.
- **+ 0.25 pts** — Trivially vague (e.g., "they measure different things").
- **+ 0 pts** — Missing.
...
- ✔ **+ 2.5 pts** — Takes a clear position (agree or disagree) and gives a reason. The bar is low: any committed stance with a reason from either paper earns full credit. Does not require a specific mechanism — even "prediction doesn't equal understanding because different architectures can predict similarly" is sufficient.
- **+ 1.75 pts** — Has a stance but the reason is purely generic with no reference to the readings (e.g., "correlation isn't causation" with no connection to Doerig or K&D).
...
- **+ 1.25 pts** — Mentions an example but it's thin or only partially explained.
- **+ 0.5 pts** — Very vague reference to readings with no specific example.
...
- **+ 2.5 pts** — Names a recognizable test from Doerig et al.'s framework AND explains what it would reveal / why it matters in detail.
- ✔ **+ 2.25 pts** — Names a real test and gives a brief explanation of what it reveals. Full credit at this tier even if the explanation is compressed — the student just needs to show they understand why this test matters beyond Brain-Score.
- **+ 1.75 pts** — Names a test but explanation of what it reveals is generic or thin (e.g., "test behavior" without saying what behavioral alignment means).
- **+ 1 pt** — Vaguely describes something related but doesn't clearly connect to Doerig's framework.
...
- ✔ **+ 0.75 pts** — Mentions a property but somewhat loosely (e.g., "has poor resolution" without specifying which kind).
- **+ 0.25 pts** — Very vague (e.g., "has limitations").
...
- ✔ **+ 0.75 pts** — Explains a bias direction but loosely or without full reasoning.
- **+ 0.25 pts** — Mentions bias but without clear direction or reasoning.
- **+ 0 pts** — Missing or incorrect.
...
- ✔ **+ 2 pts** — Writing is clear and on-topic. This is the default for any readable response where both papers are referenced somewhere (even briefly). Don't require polished prose.
- **+ 1.5 pts** — Writing is clear, response complete, but only one paper clearly referenced.
- **+ 1 pt** — Both papers referenced but writing is rushed or hard to follow in places.
...
|-------|-------------------|
| 19–20 | Every sub-component at or near top tier; both papers integrated; cross-references to earlier weeks |
| 17.5–19 | Solid answers, both papers referenced, reasonable specificity. **This is the target for a good student.** |
| 15–17 | All questions attempted, but one or more answers stay surface-level |
| 12–14.5 | Missing a question, or multiple answers too vague to show engagement |
| <12 | Minimal effort or largely off-topic |
...
*[1a: 2. 1b: 2 — captures pairwise comparison idea ("computes how similar each pair is"). 1c: 1. Subtotal: 5.]*
...
*[Test 1: 2.25 — names test with brief explanation. Test 2: 2.5 — connects to W7 causal logic. Subtotal: 4.75.]*
...
*Clarity: 2 — clear writing, both papers referenced. Total: 5 + 4.5 + 4.75 + 3 + 2 = 19.25/20.*

---

### Sample C — Score: 17.5/20
...
*[1a: 2 — mentions regression and fMRI voxels. 1b: 2 — mentions "pairwise similarity matrices," capturing the pairwise comparison idea. 1c: 0.75 — identifies a real difference (layer-specificity vs. overall structure) but explains loosely. Subtotal: 4.75.]*
...
*[2a: 2.5 — clear stance + reason grounded in Doerig. 2b: 1.25 — mentions multiple-architectures argument from Doerig but doesn't develop into a full scenario. Subtotal: 3.75.]*
...
*[Test 1: 2.25 — names behavioral alignment with brief explanation. Test 2: 2.25 — names lesion/intervention test with brief explanation. Subtotal: 4.5.]*
...
*[4a: 1 — fMRI. 4b: 0.75 — mentions temporal resolution, slightly loosely described. 4c: 0.75 — direction of bias present (overestimate) with some reasoning. Subtotal: 2.5.]*

*Clarity: 2 — writing is clear, both papers referenced (K&D throughout Q1/Q4, Doerig in Q2). Total: 4.75 + 3.75 + 4.5 + 2.5 + 2 = 17.5/20.*

---

### Sample D — Score: 14.75/20
...
*[1a: 1.5 — mentions regression but vague. 1b: 1.5 — labels RSA correctly as comparing similarity patterns, but no detail on pairwise structure. 1c: 0.5 — "more direct vs. overall shape" is vaguely correct but poorly explained. Subtotal: 3.5.]*
...
*[2a: 2.5 — stance + reason from Doerig (different architectures predict equally well). 2b: 1.25 — mentions the multiple-architectures argument but thin. Subtotal: 3.75.]*
...
*[Test 1: 1.75 — names behavioral test but explanation thin. Test 2: 1.75 — mentions OOD generalization but explanation thin. Subtotal: 3.5.]*
...
*[4a: 1 — EEG. 4b: 1 — spatial resolution. 4c: 0.75 — direction correct (misleading) with brief reasoning. Subtotal: 2.75.]*

*Clarity: 1.25 — writing is readable but rushed, both papers referenced (K&D implicitly in Q1/Q4, Doerig in Q2). Total: 3.5 + 3.75 + 3.5 + 2.75 + 1.25 = 14.75/20.*
...
- All 4 answered but one or more answers stay surface-level → score should land around **15–17**.
- Missing a question or clearly didn't engage with one paper → score should land around **12–14.5**.
- Minimal effort → score should land below **12**.
